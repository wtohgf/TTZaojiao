...
+ (WUEmoticonsKeyboard *)sharedEmoticonsKeyboard {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSMutableArray* emojlist = [NSMutableArray array];
        
        for (int i=0; i<51; i++) {
            NSString* imageName = [NSString stringWithFormat:@"expression_%02d", i+1];
            NSString* textCode = [NSString stringWithFormat:@"[expre_%02d]",i+1];

            //Icon keys
            WUEmoticonsKeyboardKeyItem *key = [[WUEmoticonsKeyboardKeyItem alloc] init];
            key.image = [UIImage imageNamed:imageName];
            key.textToInput = textCode;
            
            [emojlist addObject:key];
        }
...
//        weicoKey.textToInput = @"[weico]";
        ...
        imageIconsGroup.keyItems = emojlist;
