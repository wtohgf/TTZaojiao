//
//  TTWoGrowTestReportTigeCellTableViewCell.m
//  TTzaojiao
//
...
@implementation TTWoGrowTestReportTigeCellTableViewCell
...
+ (instancetype)WoGrowTestReportTigeCellWithTabelView:(UITableView *)tableView
{
    ...
    TTWoGrowTestReportTigeCellTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID ];
    if (nil == cell) {
        cell = [[TTWoGrowTestReportTigeCellTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
...
- (void)layoutSubviews{
    ...
    [_picView setFrame:CGRectMake(0, 30, [UIScreen mainScreen].bounds.size.width, 100)];
    if (_modelFrame == nil) {
    ...
    {
    [_descLabel setFrame:CGRectMake(8, _modelFrame.tigePicHeight+30, _modelFrame.shengaotizhongFrame.size.width,_modelFrame.shengaotizhongFrame.size.height) ];
    }
...
- (void)setModelFrame:(TTWoGrowTestReportFrame *)modelFrame
{
    ...
    _titleLabel.text = @"体格发育曲线图";
    //_picView
#pragma warning
    //需要男女，身高，体重
    NSString * gender = [TTUserModelTool sharedUserModelTool].logonUser.gender;
    if ([gender isEqualToString:@"1"] ) {
        [self test:@"shengao_tizhong_male.png" andLeftImg:@"tizhong_v.png" andBottomImg:@"shengao_h.png" andpicType:1];

    }
    else
    {
    [self test:@"shengao_tizhong_female.png" andLeftImg:@"tizhong_v.png" andBottomImg:@"shengao_h.png" andpicType:1];
    }
    
    _descLabel.text = modelFrame.model.tige_sort_content;
}
...
#pragma warning
    ...
    CGFloat pointX = picWidth/(14*5) * ([_modelFrame.model.Height floatValue]-50);
    CGFloat pointY;
    if (type == 1) {
        pointX = picWidth/(14*5) * (shengao-50);
        pointY = picHeight- picHeight
        /(18)*[_modelFrame.model.Weight floatValue]/2;
        
    }
    else if(type == 2)
    {
        pointX = picWidth/(7*6) * [_modelFrame.model.yueling floatValue] /2;
        pointY = picHeight- picHeight
        /(23)*(([_modelFrame.model.Height floatValue]-35)/5);
